...
import travelcompany.common.business.database.datasource.HikariCpDataSource;
...
public class QueryWrapper {
    ...
    public static <T> List<T> select(String sql, Connection connection, ResultSetExtractor<T> extractor)
            throws Exception {
    ...
    public static <T> List<T> select(String sql, Connection connection, ResultSetToExtractor<T> extractor)
            throws Exception {
        try (PreparedStatement ps = connection.prepareStatement(sql);
             ResultSet resultSet = ps.executeQuery()) {
            List<T> result = new ArrayList<>();
    ...
    public static <T> List<T> select(String sql, Connection connection, ResultSetExtractor<T> mapper,
                                     PreparedStatementIdentityFunc psIdentityFunc) throws Exception {
    ...
    public static <T> List<T> select(String sql, Connection connection, ResultSetExtractor<T> mapper,
                                     PreparedStatementConsumer psConsumer) throws Exception {
    ...
    public static <T> Optional<T> selectOne(String sql, Connection connection,
                                            ResultSetExtractor<T> extractor) throws Exception {
        try (PreparedStatement ps = connection.prepareStatement(sql);
             ResultSet resultSet = ps.executeQuery()) {
            if (resultSet.next()) {
    ...
    public static <T> Optional<T> selectOne(String sql, Connection connection, ResultSetExtractor<T> extractor,
                                            PreparedStatementIdentityFunc psIdentityFunc) throws Exception {
        try (PreparedStatement ps = psIdentityFunc.applyParamsAndGet(connection.prepareStatement(sql));
             ResultSet resultSet = ps.executeQuery()) {
            if (resultSet.next()) {
    ...
    public static <T> Optional<T> selectOne(String sql, Connection connection, ResultSetExtractor<T> extractor,
                                            PreparedStatementConsumer consumer) throws Exception {
        JdbcSupplier<PreparedStatement> supplier = () -> {
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    public static <T> Optional<T> executeUpdateReturningGeneratedKey(String sql, Connection connection,
        ...
        try (PreparedStatement ps = psIdentityFunc.applyParamsAndGet(connection
        ...
        if (CollectionUtils.isNotEmpty(batchItems)) {
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                for (T batchItem : batchItems) {
                    needFlush = true;
                    counter++;
                    psConsumer.consume(ps, batchItem);
                    ps.addBatch();
